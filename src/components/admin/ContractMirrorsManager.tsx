...
export function ContractMirrorsManager({ selectedCountry }: Props) {
  ...
  const fetchContracts = async () => {
    ...
    try {
      ...
      if (error) throw error;
      setContracts((data as unknown as ContractMirror[]) || []);
    } catch (error) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
      ...
      {isAddingContract && (
        <form onSubmit={handleSubmit} className="mb-6 p-4 bg-slate-50 rounded-lg">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">Data Efetiva</label>
              <input
                type="date"
          ...
          <div className="mb-4">
            <label className="block text-sm font-medium text-slate-700 mb-2">Conteúdo do Contrato</label>
            <textarea
      ...
      {loading ? (
        ...
        <div className="text-center py-8 text-slate-500">Nenhum contrato cadastrado</div>
      ) : (
        <div className="space-y-4">
          {contracts.map((contract) => (
            <div key={contract.id} className="p-4 border border-slate-200 rounded-lg hover:border-brand-green-300 transition-colors">
              <div className="flex items-start justify-between mb-3">
                ...
                <div className="flex gap-2">
                  <button onClick={() => handleEdit(contract)} className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors">
                    <Edit2 className="w-4 h-4" />
                  </button>
                  <button onClick={() => handleDelete(contract.id)} className="p-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors">
